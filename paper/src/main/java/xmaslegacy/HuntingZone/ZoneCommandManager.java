	...
	public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String @NotNull [] args) {
		...
		if (args.length == 2) {
			...
			try {
				...
				InfoUtils.infoMsg(InfoLevel.ERROR, p, "유효하지 않은 타입입니다. \"" + args[1] + "\"");
				return true;
			}
			HuntingZone zone = hzm.getZone(type);
			/*
			if (zone == null) {
				InfoUtils.infoMsg(InfoLevel.ERROR, p, "해당 사냥터가 설정되지 않았거나 적절하지 않습니다.");
				return true;
			}
			*/
			switch (args[0].toLowerCase()) {
				case "get" -> {
					...
					if (chunks.length == 0) {
						InfoUtils.infoMsg(InfoLevel.WARN, p, "해당 사냥터의 구역이 아직 설정되지 않았습니다.");
						return true;
					}
					InfoUtils.infoMsg(InfoLevel.WARN, p, "로드된 청크에 한하여 출력됩니다.");
					Arrays.stream(chunks).forEach(c -> p.sendMessage(ColorUtils.chat(String.format("&6x&f : %d, &6z&f : %d", c.getX(), c.getZ()))));
				}
				case "spawn" -> {
					if (zone.isEnabled()) {
						InfoUtils.infoMsg(InfoLevel.WARN, p, "이미 활성화되어 있습니다.");
					} else {
						InfoUtils.infoMsg(InfoLevel.INFO, p, "활성화 하였습니다.");
						zone.enable();
					}
				}
				case "despawn" -> {
					if (zone.isEnabled()) {
						InfoUtils.infoMsg(InfoLevel.INFO, p, "비활성화 하였습니다.");
						zone.disable();
					} else {
						InfoUtils.infoMsg(InfoLevel.WARN, p, "이미 비활성화 상태입니다.");
					}
				}
				case "alive" -> InfoUtils.infoMsg(InfoLevel.WARN, p, String.format("현재 사냥터 몹 수(최대마릿수 초과시 검사필요): %d/%d", zone.getAliveMobCount(), zone.getMaxSpawn()));
				case "expand" -> {
					if (chunk.isLoaded() && chunk.isGenerated()) {
						if (zone.inZone(chunk)) {
							InfoUtils.infoMsg(InfoLevel.WARN, p, "이미 포함되어 있는 청크입니다.");
						} else {
							zone.enLarge(chunk);
						}
					}
				...
				case "delete" -> {
					if (zone.inZone(chunk)) {
						zone.shrink(chunk);
						InfoUtils.infoMsg(InfoLevel.INFO, p, "해당 위치의 구역을 제외하였습니다.");
					} else {
						InfoUtils.infoMsg(InfoLevel.WARN, p, "해당 구역에 포함되지않은 청크입니다.");
					}
				}
				default -> InfoUtils.infoMsg(InfoLevel.ERROR, p, "유효하지 않은 명령어입니다.");
			}
		} else {
