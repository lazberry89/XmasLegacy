		...
		for (Entity entity : caster.getWorld().getNearbyEntities(caster.getLocation(), container.second_skill_range(), container.second_skill_range(), container.second_skill_range())) {
			if (entity instanceof LivingEntity e && !caster.equals(e)) {
				if (e instanceof Mob mob) {
					...
					Bukkit.getScheduler().runTaskLater(XmasLegacy.getInstance(), () -> {
						if (mob.isValid()) {
							mob.setAI(false);
							...
							mob.setTarget(null);
							Bukkit.getScheduler().runTaskLater(XmasLegacy.getInstance(), () -> mob.setAI(true), container.second_skill_ai_restore_delay());
						}
