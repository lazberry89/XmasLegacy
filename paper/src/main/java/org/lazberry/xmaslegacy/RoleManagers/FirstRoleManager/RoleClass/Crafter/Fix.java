...
@Skill(type = PlayerSkills.FIX)
public class Fix implements Skills<Crafter.Container>, UsingEnergy {

	@Override
	public boolean execute(@NotNull Player caster, @NotNull Crafter.Container cnt) {
		ItemStack tool = caster.getInventory().getItemInMainHand();
		if (!consumeEnergy(caster, cnt.first_skill_hunger_cost())) return false;
		...
		int repairAmount = (int) (tool.getType().getMaxDurability() * percent);
