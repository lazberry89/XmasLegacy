...
public class InquiryCommandManager implements CommandExecutor {
	...
	public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String label, @NotNull String... args) {
		...
		if (IM.checkAndSetCooldown(p.getUniqueId())) {
			...
			p.sendMessage(ColorUtils.chat(STR."\{Prefix.RED} 아직 문의할 수 없습니다. (\{secondsLeft}초 남음)"));
			return true;
		}

		if (args.length == 0) {
			IM.Inquiry(p.getUniqueId(), null);
			p.sendMessage(ColorUtils.chat(STR."\{Prefix.YELLOW} 운영자를 호출했습니다. 조금만 기다려주세요!"));
		} else {
			String content = String.join(" ", args);
			IM.Inquiry(p.getUniqueId(), content);
			p.sendMessage(ColorUtils.chat(STR."\{Prefix.YELLOW} 문의가 접수되었습니다."));
		}
