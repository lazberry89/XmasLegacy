		...
		if (args.length == 1) {
			...
			if (target != null && target.isOnline()) {
				if (IM.hasInquiry(target.getUniqueId())) {
					...
					p.sendMessage(ColorUtils.chat(STR."\{Prefix.YELLOW} 이동하였습니다."));
					p.playSound(p, Sound.ENTITY_ARROW_HIT_PLAYER, 1.0f, 1.0f);

					target.sendMessage(ColorUtils.chat(STR."\{Prefix.YELLOW} 관리자가 배정되었습니다 : &c&l\{p.getName()}"));
					...
					p.sendMessage(ColorUtils.chat(STR."\{Prefix.RED} 해당 유저의 문의가 처리되었거나 없습니다."));
				}
			}
		}
		return true;
